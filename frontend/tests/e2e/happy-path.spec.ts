...
test("enroll, complete diagnostic, and attempt a gate", async ({ page }) => {
  ...
  await page.getByRole("link", { name: /ready for the gate quiz/i }).click();
  ...
  for (let i = 0; i < 5; i++) {
    await page.getByRole("radio").first().check();
    await page
      .getByRole("button", { name: /next question|submit quiz/i })
      .click();
  }

  // Either outcome is acceptable; both route somewhere sensible.
  await expect(
    page.getByText(/passed!|not yet — let's review/i),
  ).toBeVisible();
});
